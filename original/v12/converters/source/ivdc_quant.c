/*
 *  ivdc_quant.c - written 2011-04-09 by Michael Kircher
 *
 *  Quantizes a 640x480 *.ppm picture to a interlaced VDC bitmap with 8x2 attributes
 *
 *       Date of change: 2011-04-09
 *  - Attributes corrected for interleaved positioning
 *  Date of last change: 2011-04-12
 *  - *.bin output changed to *.prg including start address at $8000
 */
...
void quant_column(VDC_COLUMN *screen, IMAGE *dst, IMAGE *src, SIZE x)
{
  ...
  {
    fprintf(stderr,"column %d, half-char %d ...\n",x>>3,row>>1);
    z=4;
...
int main(int argc, char *argv[])
{
  ...
  /* write out VDC data as *.prg files starting at $8000 */
